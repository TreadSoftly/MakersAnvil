...
7. The latest pass report named by `currentPass.reportPath`
...
6. Update file-level explanations, public docstrings, reasoning comments, tests, architecture, and `state/source_manifest.json` for every responsibility changed.
...
- Every Python module and public component must satisfy the docstring checks.
- Frontend and workflow files must retain purpose and section comments.
